...
const FunctionalReachTest = ({ selectedOption , selectedYear }) => {
  ...
  const exportToExcel = () => {
    ...
    const formattedDate = currentDate
      ...
      .replace(/\//g, "_");
    const fileName = `FunctionalReachTest_${formattedDate}`;
    ...
    details.forEach((rowData) => {
      ...
      rowData.disciplineTypeData.forEach((innerrowData) => {
        ...
        functionalOutcomeMeasureTest.forEach((nestedRowData) => {
            const {name , fields} = nestedRowData;
            if (count == 0) {
                // const skillRow = [
                //     disciplineType,
                ...
                //   rowIndex++;
                let innercount = 0;
            nestedRowData.fields.commonFields.forEach((feildrowData) => {
            if(innercount == 0){
            const skillRow = [
                disciplineType,
                functionalOutcomeMeasureType[0]+" , "+functionalOutcomeMeasureType[1],
              ...
              rowIndex++;
                ...
                const skillRow = [
                    ...
                    ' ',
                    nestedRowData.name,
                    nestedRowData.fields.imagesInfo,
                    nestedRowData.fields.description
                  ];
                  XLSX.utils.sheet_add_aoa(ws, [skillRow], {
                    origin: `A${rowIndex}`,
                  });
...
export default FunctionalReachTest;
